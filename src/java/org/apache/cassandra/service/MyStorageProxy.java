...
public class MyStorageProxy implements StorageProxyMBean {
	...
	public static final MyStorageProxy instance = new MyStorageProxy();
	...
	private static final Map<ConsistencyLevel, ClientRequestMetrics> readMetricsMap = new EnumMap<>(
			ConsistencyLevel.class);
	private static final Map<ConsistencyLevel, ClientRequestMetrics> writeMetricsMap = new EnumMap<>(
			ConsistencyLevel.class);

	private static final double CONCURRENT_SUBREQUESTS_MARGIN = 0.10;

	private MyStorageProxy() {
	}
	...
	static {
		...
		standardWritePerformer = new WritePerformer() {
			public void apply(IMutation mutation, Iterable<InetAddress> targets,
					AbstractWriteResponseHandler<IMutation> responseHandler, String localDataCenter,
		...
		/*
		 * We execute counter writes in 2 places: either directly in the coordinator
		 * node if it is a replica, or in CounterMutationVerbHandler on a replica
		 * othewise. The write must be executed on the COUNTER_MUTATION stage but on the
		 * latter case, the verb handler already run on the COUNTER_MUTATION stage, so
		 * we must not execute the underlying on the stage otherwise we risk a deadlock.
		 * Hence two different performer.
		 */
	...
	/**
	 * Apply @param updates if and only if the current values in the row for @param
	 * key match the provided @param conditions. The algorithm is "raw" Paxos: that
	 * is, Paxos minus leader election -- any node in the cluster may propose
	 * changes for any row, which (that is, the row) is the unit of values being
	 * proposed, not single columns.
	 *
	 * The Paxos cohort is only the replicas for the given key, not the entire
	 * cluster. So we expect performance to be reasonable, but CAS is still intended
	 * to be used "when you really need it," not for all your updates.
	 *
	 * There are three phases to Paxos: 1. Prepare: the coordinator generates a
	 * ballot (timeUUID in our case) and asks replicas to (a) promise not to accept
	 * updates from older ballots and (b) tell us about the most recent update it
	 * has already accepted. 2. Accept: if a majority of replicas reply, the
	 * coordinator asks replicas to accept the value of the highest proposal ballot
	 * it heard about, or a new value if no in-progress proposals were reported. 3.
	 * Commit (Learn): if a majority of replicas acknowledge the accept request, we
	 * can commit the new value.
	 *
	 * Commit procedure is not covered in "Paxos Made Simple," and only briefly
	 * mentioned in "Paxos Made Live," so here is our approach: 3a. The coordinator
	 * sends a commit message to all replicas with the ballot and value. 3b. Because
	 * of 1-2, this will be the highest-seen commit ballot. The replicas will note
	 * that, and send it with subsequent promise replies. This allows us to discard
	 * acceptance records for successfully committed replicas, without allowing
	 ...
	 * perform a read (of committed values) between the prepare and accept phases.
	 * This gives us a slightly longer window for another coordinator to come along
	 * and trump our own promise with a newer one but is otherwise safe.
	 *
	 * @param keyspaceName
	 ...
	 * @param request
	 *            the conditions for the CAS to apply as well as the update to
	 *            perform if the conditions hold.
	 * @param consistencyForPaxos
	 *            the consistency for the paxos prepare and propose round. This can
	 *            only be either SERIAL or LOCAL_SERIAL.
	 * @param consistencyForCommit
	 *            the consistency for write done during the commit phase. This can
	 *            be anything, except SERIAL or LOCAL_SERIAL.
	 *
	 * @return null if the operation succeeds in updating the row, or the current
	 *         values corresponding to conditions. (since, if the CAS doesn't
	 *         succeed, it means the current value do not match the conditions).
	 */
	public static RowIterator cas(String keyspaceName, String cfName, DecoratedKey key, CASRequest request,
			ConsistencyLevel consistencyForPaxos, ConsistencyLevel consistencyForCommit, ClientState state,
			long queryStartNanoTime) throws UnavailableException, IsBootstrappingException, RequestFailureException,
			RequestTimeoutException, InvalidRequestException {
		...
		try {
			...
			while (System.nanoTime() - queryStartNanoTime < timeout) {
				...
				final Pair<UUID, Integer> pair = beginAndRepairPaxos(queryStartNanoTime, key, metadata, liveEndpoints,
				...
				ConsistencyLevel readConsistency = consistencyForPaxos == ConsistencyLevel.LOCAL_SERIAL
	...
	private static Pair<List<InetAddress>, Integer> getPaxosParticipants(CFMetaData cfm, DecoratedKey key,
			ConsistencyLevel consistencyForPaxos) throws UnavailableException {
		...
		List<InetAddress> liveEndpoints = ImmutableList
				.copyOf(Iterables.filter(Iterables.concat(naturalEndpoints, pendingEndpoints), IAsyncCallback.isAlive));
		if (liveEndpoints.size() < requiredParticipants)
			throw new UnavailableException(consistencyForPaxos, requiredParticipants, liveEndpoints.size());

		// We cannot allow CAS operations with 2 or more pending endpoints, see #8346.
		// Note that we fake an impossible number of required nodes in the unavailable
		// exception
		// to nail home the point that it's an impossible operation no matter how many
		// nodes are live.
		if (pendingEndpoints.size() > 1)
			throw new UnavailableException(
					String.format("Cannot perform LWT operation as there is more than one (%d) pending range movement",
							pendingEndpoints.size()),
					consistencyForPaxos, participants + 1, liveEndpoints.size());
	 ...
	 *
	 * @return the Paxos ballot promised by the replicas if no in-progress requests
	 *         were seen and a quorum of nodes have seen the mostRecentCommit.
	 *         Otherwise, return null.
	 */
	private static Pair<UUID, Integer> beginAndRepairPaxos(long queryStartNanoTime, DecoratedKey key,
			CFMetaData metadata, List<InetAddress> liveEndpoints, int requiredParticipants,
			ConsistencyLevel consistencyForPaxos, ConsistencyLevel consistencyForCommit, final boolean isWrite,
			ClientState state) throws WriteTimeoutException, WriteFailureException {
		long timeout = TimeUnit.MILLISECONDS.toNanos(DatabaseDescriptor.getCasContentionTimeout());

		PrepareCallback summary = null;
		int contentions = 0;
		while (System.nanoTime() - queryStartNanoTime < timeout) {
			// We want a timestamp that is guaranteed to be unique for that node (so that
			// the ballot is globally unique), but if we've got a prepare rejected
			// already we also want to make sure we pick a timestamp that has a chance to be
			// promised, i.e. one that is greater that the most recently known
			// in progress (#5667). Lastly, we don't want to use a timestamp that is older
			// than the last one assigned by ClientState or operations may appear
			// out-of-order (#7801).
			...
			// Note that ballotMicros is not guaranteed to be unique if two proposal are
			// being handled concurrently by the same coordinator. But we still
			...
			if (!inProgress.update.isEmpty() && inProgress.isAfter(mostRecent)) {
				...
				if (proposePaxos(refreshedInProgress, liveEndpoints, requiredParticipants, false, consistencyForPaxos,
					...
					try {
						...
						recordCasContention(contentions);
						// We're still doing preparation for the paxos rounds, so we want to use the CAS
						// (see CASSANDRA-8672)
			...
			// To be able to propose our value on a new round, we need a quorum of replica
			// to have learn the previous one. Why is explained at:
			// https://issues.apache.org/jira/browse/CASSANDRA-5062?focusedCommentId=13619810&page=com.atlassian.jira.plugin.system.issuetabpanels:comment-tabpanel#comment-13619810)
			// Since we waited for quorum nodes, if some of them haven't seen the last
			// commit (which may just be a timing issue, but may also
			...
			if (Iterables.size(missingMRC) > 0) {
				...
				// TODO: provided commits don't invalid the prepare we just did above (which
				// they don't), we could just wait
				// for all the missingMRC to acknowledge this commit and then move on with
				// proposing our value. But that means
				// adding the ability to have commitPaxos block, which is exactly CASSANDRA-5442
				// will do. So once we have that
				// latter ticket, we can pass CL.ALL to the commit above and remove the
				// 'continue'.
				continue;
			}
	...
	private static void sendCommit(Commit commit, Iterable<InetAddress> replicas) {
		MessageOut<Commit> message = new MessageOut<Commit>(MessagingService.Verb.PAXOS_COMMIT, commit,
				Commit.serializer);
	...
	private static PrepareCallback preparePaxos(Commit toPrepare, List<InetAddress> endpoints, int requiredParticipants,
			ConsistencyLevel consistencyForPaxos, long queryStartNanoTime) throws WriteTimeoutException {
		PrepareCallback callback = new PrepareCallback(toPrepare.update.partitionKey(), toPrepare.update.metadata(),
				requiredParticipants, consistencyForPaxos, queryStartNanoTime);
		MessageOut<Commit> message = new MessageOut<Commit>(MessagingService.Verb.PAXOS_PREPARE, toPrepare,
				Commit.serializer);
	...
	private static boolean proposePaxos(Commit proposal, List<InetAddress> endpoints, int requiredParticipants,
			boolean timeoutIfPartial, ConsistencyLevel consistencyLevel, long queryStartNanoTime)
		...
		MessageOut<Commit> message = new MessageOut<Commit>(MessagingService.Verb.PAXOS_PROPOSE, proposal,
				Commit.serializer);
		for (InetAddress target : endpoints)
			MessagingService.instance().sendRR(message, target, callback);
	...
	private static void commitPaxos(Commit proposal, ConsistencyLevel consistencyLevel, boolean allowHints,
		...
		if (shouldBlock) {
			...
			responseHandler = rs.getWriteResponseHandler(naturalEndpoints, pendingEndpoints, consistencyLevel, null,
					WriteType.SIMPLE, queryStartNanoTime);
			responseHandler.setSupportsBackPressure(false);
		}

		MessageOut<Commit> message = new MessageOut<Commit>(MessagingService.Verb.PAXOS_COMMIT, proposal,
				Commit.serializer);
				...
				if (shouldBlock) {
					if (canDoLocalRequest(destination))
						...
						MessagingService.instance().sendRR(message, destination, responseHandler,
								allowHints && shouldHint(destination));
				} else {
	...
	/**
	 * Commit a PAXOS task locally, and if the task times out rather then submitting
	 * a real hint submit a fake one that executes immediately on the mutation
	 * stage, but generates the necessary backpressure signal for hints
	 */
	private static void commitPaxosLocal(final MessageOut<Commit> message,
			final AbstractWriteResponseHandler<?> responseHandler) {
		StageManager.getStage(MessagingService.verbStages.get(MessagingService.Verb.PAXOS_COMMIT))
				.maybeExecuteImmediately(new LocalMutationRunnable() {
	 ...
	 * method will take care of the possibility of a replica being down and hint the
	 * data across to some other replica.
	 *
	 * @param mutations
	 ...
	 *            the value of System.nanoTime() when the query started to be
	 *            processed
	 */
		...
		try {
			for (IMutation mutation : mutations) {
				if (mutation instanceof CounterMutation) {
					responseHandlers
							.add(mutateCounter((CounterMutation) mutation, localDataCenter, queryStartNanoTime));
				} else {
					WriteType wt = mutations.size() <= 1 ? WriteType.SIMPLE : WriteType.UNLOGGED_BATCH;
					responseHandlers.add(performWrite(mutation, consistency_level, localDataCenter,
							standardWritePerformer, null, wt, queryStartNanoTime));
				}
			}
			...
			if (consistency_level == ConsistencyLevel.ANY) {
				...
				if (ex instanceof WriteFailureException) {
					...
					Tracing.trace("Write failure; received {} of {} required replies, failed {} requests", fe.received,
							fe.blockFor, fe.failureReasonByEndpoint.size());
				} else {
	 ...
	 * means we'll re-hint any successful ones; doesn't seem worth it to track
	 * individual success just for this unusual case.
	 *
	 * Only used for CL.ANY
	 *
	 * @param mutations
	 *            the mutations that require hints
	 */
	 ...
	 *            if commitlog should be written
	 * @param baseComplete
	 *            time from epoch in ms that the local base mutation was(or will be)
	 *            completed
	 * @param queryStartNanoTime
	 *            the value of System.nanoTime() when the query started to be
	 *            processed
	...
	public static void mutateMV(ByteBuffer dataKey, Collection<Mutation> mutations, boolean writeCommitLog,
			AtomicLong baseComplete, long queryStartNanoTime)
		...
		try {
			...
			if (StorageService.instance.isStarting() || StorageService.instance.isJoining()
				...
				// non-local mutations rely on the base mutation commit-log entry for eventual
				// consistency
				...
				// Since the base -> view replication is 1:1 we only need to store the BL
				// locally
				final Collection<InetAddress> batchlogEndpoints = Collections
						.singleton(FBUtilities.getBroadcastAddress());
				BatchlogResponseHandler.BatchlogCleanup cleanup = new BatchlogResponseHandler.BatchlogCleanup(
						mutations.size(), () -> asyncRemoveFromBatchlog(batchlogEndpoints, batchUUID));
				...
				for (Mutation mutation : mutations) {
					...
					Optional<InetAddress> pairedEndpoint = ViewUtils.getViewNaturalEndpoint(keyspaceName, baseToken,
							tk);
					...
					if (!pairedEndpoint.isPresent()) {
						if (pendingEndpoints.isEmpty())
							logger.warn("Received base materialized view mutation for key {} that does not belong "
									...
									+ "local batchlog to be replayed later.", mutation.key());
						continue;
					}
					...
					if (pairedEndpoint.get().equals(FBUtilities.getBroadcastAddress())
							&& StorageService.instance.isJoined() && pendingEndpoints.isEmpty()) {
						try {
						...
						wrappers.add(wrapViewBatchResponseHandler(mutation, consistencyLevel, consistencyLevel,
								Collections.singletonList(pairedEndpoint.get()), baseComplete, WriteType.BATCH, cleanup,
								queryStartNanoTime));
					}
				}
				...
				if (!nonLocalMutations.isEmpty())
					BatchlogManager.store(
							Batch.createLocal(batchUUID, FBUtilities.timestampMicros(), nonLocalMutations),
							writeCommitLog);
	...
	public static void mutateWithTriggers(Collection<? extends IMutation> mutations, ConsistencyLevel consistencyLevel,
			boolean mutateAtomically, long queryStartNanoTime) throws WriteTimeoutException, WriteFailureException,
			UnavailableException, OverloadedException, InvalidRequestException {
		Collection<Mutation> augmented = TriggerExecutor.instance.execute(mutations);

		boolean updatesView = Keyspace.open(mutations.iterator().next().getKeyspaceName()).viewManager
	...
	/**
	 * See mutate. Adds additional steps before and after writing a batch. Before
	 * writing the batch (but after doing availability check against the FD for the
	 * row replicas): write the entire batch to a batchlog elsewhere in the cluster.
	 ...
	 * @param consistency_level
	 *            the consistency level for the operation
	 * @param requireQuorumForRemove
	 *            at least a quorum of nodes will see update before deleting
	 *            batchlog
	 * @param queryStartNanoTime
	 *            the value of System.nanoTime() when the query started to be
	 *            processed
	...
	public static void mutateAtomically(Collection<Mutation> mutations, ConsistencyLevel consistency_level,
			boolean requireQuorumForRemove, long queryStartNanoTime)
			throws UnavailableException, OverloadedException, WriteTimeoutException {
		...
		try {
			...
			// If we are requiring quorum nodes for removal, we upgrade consistency level to
			// QUORUM unless we already
			// require ALL, or EACH_QUORUM. This is so that *at least* QUORUM nodes see the
			// update.
			ConsistencyLevel batchConsistencyLevel = requireQuorumForRemove ? ConsistencyLevel.QUORUM
					: consistency_level;
			...
			final UUID batchUUID = UUIDGen.getTimeUUID();
			BatchlogResponseHandler.BatchlogCleanup cleanup = new BatchlogResponseHandler.BatchlogCleanup(
					mutations.size(), () -> asyncRemoveFromBatchlog(batchlogEndpoints, batchUUID, queryStartNanoTime));

			// add a handler for each mutation - includes checking availability, but doesn't
			// initiate any writes, yet
			for (Mutation mutation : mutations) {
				WriteResponseHandlerWrapper wrapper = wrapBatchResponseHandler(mutation, consistency_level,
						batchConsistencyLevel, WriteType.BATCH, cleanup, queryStartNanoTime);
	...
	private static void syncWriteToBatchlog(Collection<Mutation> mutations, BatchlogEndpoints endpoints, UUID uuid,
			long queryStartNanoTime) throws WriteTimeoutException, WriteFailureException {
		WriteResponseHandler<?> handler = new WriteResponseHandler<>(endpoints.all,
				...
				Keyspace.open(SchemaConstants.SYSTEM_KEYSPACE_NAME), null, WriteType.BATCH_LOG, queryStartNanoTime);
	...
	private static void syncWriteToBatchlog(WriteResponseHandler<?> handler, Batch batch,
			Collection<InetAddress> endpoints) throws WriteTimeoutException, WriteFailureException {
	...
	private static void asyncRemoveFromBatchlog(Collection<InetAddress> endpoints, UUID uuid) {
		MessageOut<UUID> message = new MessageOut<>(MessagingService.Verb.BATCH_REMOVE, uuid,
	...
	private static void syncWriteBatchedMutations(List<WriteResponseHandlerWrapper> wrappers, String localDataCenter,
			Stage stage) throws WriteTimeoutException, OverloadedException {
		for (WriteResponseHandlerWrapper wrapper : wrappers) {
			Iterable<InetAddress> endpoints = Iterables.concat(wrapper.handler.naturalEndpoints,
					wrapper.handler.pendingEndpoints);
			sendToHintedEndpoints(wrapper.mutation, endpoints, wrapper.handler, localDataCenter, stage);
	...
	/**
	 * Perform the write of a mutation given a WritePerformer. Gather the list of
	 * write endpoints, apply locally and/or forward the mutation to said write
	 * endpoint (deletaged to the actual WritePerformer) and wait for the responses
	 ...
	 * @param performer
	 *            the WritePerformer in charge of appliying the mutation given the
	 *            list of write endpoints (either standardWritePerformer for
	...
	public static AbstractWriteResponseHandler<IMutation> performWrite(IMutation mutation,
			ConsistencyLevel consistency_level, String localDataCenter, WritePerformer performer, Runnable callback,
			WriteType writeType, long queryStartNanoTime) throws UnavailableException, OverloadedException {
		String keyspaceName = mutation.getKeyspaceName();
		...
		AbstractWriteResponseHandler<IMutation> responseHandler = rs.getWriteResponseHandler(naturalEndpoints,
				pendingEndpoints, consistency_level, callback, writeType, queryStartNanoTime);
	...
	private static WriteResponseHandlerWrapper wrapBatchResponseHandler(Mutation mutation,
			ConsistencyLevel consistency_level, ConsistencyLevel batchConsistencyLevel, WriteType writeType,
			BatchlogResponseHandler.BatchlogCleanup cleanup, long queryStartNanoTime) {
		...
		AbstractWriteResponseHandler<IMutation> writeHandler = rs.getWriteResponseHandler(naturalEndpoints,
				pendingEndpoints, consistency_level, null, writeType, queryStartNanoTime);
		BatchlogResponseHandler<IMutation> batchHandler = new BatchlogResponseHandler<>(writeHandler,
	...
	/**
	 * Same as performWrites except does not initiate writes (but does perform
	 * availability checks). Keeps track of ViewWriteMetrics
	 */
	private static WriteResponseHandlerWrapper wrapViewBatchResponseHandler(Mutation mutation,
			ConsistencyLevel consistency_level, ConsistencyLevel batchConsistencyLevel,
			List<InetAddress> naturalEndpoints, AtomicLong baseComplete, WriteType writeType,
			BatchlogResponseHandler.BatchlogCleanup cleanup, long queryStartNanoTime) {
		Keyspace keyspace = Keyspace.open(mutation.getKeyspaceName());
		AbstractReplicationStrategy rs = keyspace.getReplicationStrategy();
		String keyspaceName = mutation.getKeyspaceName();
	...
	// used by atomic_batch_mutate to decouple availability check from the write
	// itself, caches consistency level and endpoints.
	...
	/*
	 * A class to filter batchlog endpoints into legacy endpoints (version < 3.0) or
	 * not.
	 */
	...
	/*
	 * Replicas are picked manually: - replicas should be alive according to the
	 * failure detector - replicas should be in the local datacenter - choose min(2,
	 * number of qualifying candiates above) - allow the local node to be the only
	 * replica only if it's a single-node DC
	 */
	private static BatchlogEndpoints getBatchlogEndpoints(String localDataCenter, ConsistencyLevel consistencyLevel)
		...
		Multimap<String, InetAddress> localEndpoints = HashMultimap
				.create(topology.getDatacenterRacks().get(localDataCenter));
		String localRack = DatabaseDescriptor.getEndpointSnitch().getRack(FBUtilities.getBroadcastAddress());

		Collection<InetAddress> chosenEndpoints = new BatchlogManager.EndpointFilter(localRack, localEndpoints)
				.filter();
	...
	/**
	 * Send the mutations to the right targets, write it locally if it corresponds
	 * or writes a hint when the node is not available.
	 *
	 * Note about hints:
	 * 
	 * <pre>
	 ...
	 */
	public static void sendToHintedEndpoints(final Mutation mutation, Iterable<InetAddress> targets,
	...
	private static void checkHintOverload(InetAddress destination) {
		...
		if (StorageMetrics.totalHintsInProgress.getCount() > maxHintsInProgress
			...
			throw new OverloadedException(
					"Too many in flight hints: " + StorageMetrics.totalHintsInProgress.getCount() + " destination: "
							+ destination + " destination hints: " + getHintsInProgressFor(destination).get());
		}
	}

	private static void sendMessagesToNonlocalDC(MessageOut<? extends IMutation> message,
			Collection<InetAddress> targets, AbstractWriteResponseHandler<IMutation> handler) {
	...
	private static void performLocally(Stage stage, Optional<IMutation> mutation, final Runnable runnable,
	 ...
	 *
	 * A counter mutation needs to first be applied to a replica (that we'll call
	 * the leader for the mutation) before being replicated to the other endpoint.
	 * To achieve so, there is two case: 1) the coordinator host is a replica: we
	 * proceed to applying the update locally and replicate throug
	 * applyCounterMutationOnCoordinator 2) the coordinator is not a replica: we
	 * forward the (counter)mutation to a chosen replica (that will proceed through
	 * applyCounterMutationOnLeader upon receive) and wait for its acknowledgment.
	 *
	 * Implementation note: We check if we can fulfill the CL on the coordinator
	 * host even if he is not a replica to allow quicker response and because the
	 * WriteResponseHandlers don't make it easy to send back an error. We also
	 * always gather the write latencies at the coordinator node to make gathering
	 * point similar to the case of standard writes.
	 */
	public static AbstractWriteResponseHandler<IMutation> mutateCounter(CounterMutation cm, String localDataCenter,
			long queryStartNanoTime) throws UnavailableException, OverloadedException {
		...
		if (endpoint.equals(FBUtilities.getBroadcastAddress())) {
			...
			AbstractWriteResponseHandler<IMutation> responseHandler = new WriteResponseHandler<>(endpoint,
	 ...
	 * alive in the local DC). TODO: if we track the latency of the counter writes
	 * (which makes sense contrarily to standard writes since there is a read
	 * involved), we could trust the dynamic snitch entirely, which may be a better
	 * solution. It is unclear we want to mix those latencies with read latencies,
	 * so this may be a bit involved.
	 */
	private static InetAddress findSuitableEndpoint(String keyspaceName, DecoratedKey key, String localDataCenter,
	...
	public static AbstractWriteResponseHandler<IMutation> applyCounterMutationOnLeader(CounterMutation cm,
			String localDataCenter, Runnable callback, long queryStartNanoTime)
			throws UnavailableException, OverloadedException {
		return performWrite(cm, cm.consistency(), localDataCenter, counterWritePerformer, callback, WriteType.COUNTER,
				queryStartNanoTime);
	}

	// Same as applyCounterMutationOnLeader but must with the difference that it use
	// the MUTATION stage to execute the write (while
	// applyCounterMutationOnLeader assumes it is on the MUTATION stage already)
	public static AbstractWriteResponseHandler<IMutation> applyCounterMutationOnCoordinator(CounterMutation cm,
			String localDataCenter, long queryStartNanoTime) throws UnavailableException, OverloadedException {
		return performWrite(cm, cm.consistency(), localDataCenter, counterWriteOnCoordinatorPerformer, null,
				WriteType.COUNTER, queryStartNanoTime);
	}

	private static Runnable counterWriteTask(final IMutation mutation, final Iterable<InetAddress> targets,
			final AbstractWriteResponseHandler<IMutation> responseHandler, final String localDataCenter) {
	...
	public static RowIterator readOne(SinglePartitionReadCommand command, ConsistencyLevel consistencyLevel,
			long queryStartNanoTime) throws UnavailableException, IsBootstrappingException, ReadFailureException,
			ReadTimeoutException, InvalidRequestException {
		return readOne(command, consistencyLevel, null, queryStartNanoTime);
	}

	public static RowIterator readOne(SinglePartitionReadCommand command, ConsistencyLevel consistencyLevel,
			ClientState state, long queryStartNanoTime) throws UnavailableException, IsBootstrappingException,
			ReadFailureException, ReadTimeoutException, InvalidRequestException {
		return PartitionIterators.getOnlyElement(
				read(SinglePartitionReadCommand.Group.one(command), consistencyLevel, state, queryStartNanoTime),
				command);
	}
	...
	/**
	 * Performs the actual reading of a row out of the StorageService, fetching a
	 * specific set of column names from a given column family.
	 */
	public static PartitionIterator read(SinglePartitionReadCommand.Group group, ConsistencyLevel consistencyLevel,
			ClientState state, long queryStartNanoTime) throws UnavailableException, IsBootstrappingException,
			ReadFailureException, ReadTimeoutException, InvalidRequestException {
	...
	private static PartitionIterator readWithPaxos(SinglePartitionReadCommand.Group group,
			ConsistencyLevel consistencyLevel, ClientState state, long queryStartNanoTime)
		...
		try {
			// make sure any in-progress paxos writes are done (i.e., committed to a
			// majority of replicas), before performing a quorum read
			Pair<List<InetAddress>, Integer> p = getPaxosParticipants(metadata, key, consistencyLevel);
			List<InetAddress> liveEndpoints = p.left;
			int requiredParticipants = p.right;

			// does the work of applying in-progress writes; throws UAE or timeout if it
			// can't
			...
			try {
				final Pair<UUID, Integer> pair = beginAndRepairPaxos(start, key, metadata, liveEndpoints,
						requiredParticipants, consistencyLevel, consistencyForCommitOrFetch, false, state);
				if (pair.right > 0)
				...
				throw new ReadFailureException(consistencyLevel, e.received, e.blockFor, false,
						e.failureReasonByEndpoint);
			}
			...
			Keyspace.open(metadata.ksName).getColumnFamilyStore(metadata.cfName).metric.coordinatorReadLatency
	...
	private static PartitionIterator readRegular(SinglePartitionReadCommand.Group group,
		...
		try {
			...
			// Note that the only difference between the command in a group must be the
			// partition key on which
			// they applied.
			boolean enforceStrictLiveness = group.commands.get(0).metadata().enforceStrictLiveness();
			// If we have more than one command, then despite each read command honoring the
	 ...
	 * a) all the replicas, if read repair is enabled b) the closest R-1 replicas,
	 * where R is the number required to satisfy the ConsistencyLevel 3. Wait for a
	 * response from R replicas 4. If the digests (if any) match the data return the
	 * data 5. else carry out read repair by getting data from all the nodes.
	 */
	...
	private static class SinglePartitionReadLifecycle {
		...
		void awaitResultsAndRetryOnDigestMismatch() throws ReadFailureException, ReadTimeoutException {
			try {
				...
				DataResolver resolver = new DataResolver(keyspace, command, ConsistencyLevel.ALL,
						executor.handler.endpoints.size(), queryStartNanoTime);
				repairHandler = new ReadCallback(resolver, ConsistencyLevel.ALL, executor.getContactedReplicas().size(),
						command, keyspace, executor.handler.endpoints, queryStartNanoTime);

				for (InetAddress endpoint : executor.getContactedReplicas()) {
					MessageOut<ReadCommand> message = command
							.createMessage(MessagingService.instance().getVersion(endpoint));
		...
		void maybeAwaitFullDataRead() throws ReadTimeoutException {
			...
			// Otherwise, get the result from the full-data read and check that it's not a
			// short read
			try {
	...
	static class LocalReadRunnable extends DroppableRunnable {
		...
		protected void runMayThrow() {
			try {
				command.setMonitoringTime(constructionTime, false, verb.getTimeout(),
	...
	private static List<InetAddress> intersection(List<InetAddress> l1, List<InetAddress> l2) {
		...
		// 1) retainAll would be inefficient if l1 and l2 are large but in practice both
		// are the replicas for a range and
		// so will be very small (< RF). In that case, retainAll is in fact more
		// efficient.
		// 2) we do ultimately need a list so converting everything to sets don't make
		// sense
		// 3) l1 and l2 are sorted by proximity. The use of retainAll maintain that
		// sorting in the result, while using sets wouldn't.
	...
	/**
	 * Estimate the number of result rows (either cql3 rows or "thrift" rows, as
	 * called for by the command) per range in the ring based on our local data.
	...
	private static float estimateResultsPerRange(PartitionRangeReadCommand command, Keyspace keyspace) {
		...
		float maxExpectedResults = index == null ? command.limits().estimateTotalResults(cfs)
	...
	private static class RangeForQuery {
		...
		public RangeForQuery(AbstractBounds<PartitionPosition> range, List<InetAddress> liveEndpoints,
	...
	private static class RangeIterator extends AbstractIterator<RangeForQuery> {
		...
		public RangeIterator(PartitionRangeReadCommand command, Keyspace keyspace, ConsistencyLevel consistency) {
			...
			List<? extends AbstractBounds<PartitionPosition>> l = keyspace
					.getReplicationStrategy() instanceof LocalStrategy ? command.dataRange().keyRange().unwrap()
	...
	private static class RangeMerger extends AbstractIterator<RangeForQuery> {
		...
		protected RangeForQuery computeNext() {
			...
			while (ranges.hasNext()) {
				...
				// Note: it would be slightly more efficient to have CFS.getRangeSlice on the
				// destination nodes unwraps
				// the range if necessary and deal with it. However, we can't start sending
				// wrapped range without breaking
	...
	private static class RangeCommandIterator extends AbstractIterator<RowIterator> implements PartitionIterator {
		...
		public RangeCommandIterator(RangeIterator ranges, PartitionRangeReadCommand command, int concurrencyFactor,
				Keyspace keyspace, ConsistencyLevel consistency, long queryStartNanoTime) {
			this.command = command;
			...
			try {
				while (sentQueryIterator == null || !sentQueryIterator.hasNext()) {
					...
					if (sentQueryIterator != null) {
						...
						// It's not the first batch of queries and we're not done, so we we can use what
						// has been
						// returned so far to improve our rows-per-range estimate and update the
		...
		private void updateConcurrencyFactor() {
			...
			// Otherwise, compute how many rows per range we got on average and pick a
			// concurrency factor
			// that should allow us to fetch all remaining rows with the next batch of
			// (concurrent) queries.
			int remainingRows = command.limits().count() - liveReturned;
			float rowsPerRange = (float) liveReturned / (float) rangesQueried;
			concurrencyFactor = Math.max(1,
					Math.min(totalRangeCount - rangesQueried, Math.round(remainingRows / rowsPerRange)));
			logger.trace(
		 ...
		 *
		 * @param toQuery
		 *            the subRange to query.
		 * @param isFirst
		 *            in the case where multiple queries are sent in parallel, whether
		 *            that's the first query on that batch or not. The reason it matters
		 *            is that whe paging queries, the command (more specifically the
		 *            {@code DataLimits}) may have "state" information and that state
		 *            may only be valid for the first query (in that it's the query that
		 *            "continues" whatever we're previously queried).
		 */
		private SingleRangeResponse query(RangeForQuery toQuery, boolean isFirst) {
			...
			DataResolver resolver = new DataResolver(keyspace, rangeCommand, consistency,
					toQuery.filteredEndpoints.size(), queryStartNanoTime);
			...
			if (toQuery.filteredEndpoints.size() == 1 && canDoLocalRequest(toQuery.filteredEndpoints.get(0))) {
				...
				for (InetAddress endpoint : toQuery.filteredEndpoints) {
					MessageOut<ReadCommand> message = rangeCommand
							.createMessage(MessagingService.instance().getVersion(endpoint));
	...
	public static PartitionIterator getRangeSlice(PartitionRangeReadCommand command, ConsistencyLevel consistencyLevel,
		...
		int concurrencyFactor = resultsPerRange == 0.0 ? 1
				: Math.max(1,
						Math.min(ranges.rangeCount(), (int) Math.ceil(command.limits().count() / resultsPerRange)));
		logger.trace(
		...
		Tracing.trace("Submitting range requests on {} ranges with a concurrency of {} ({} rows per range expected)",
				ranges.rangeCount(), concurrencyFactor, resultsPerRange);
		...
		return command.limits()
				.filter(command.postReconciliationProcessing(new RangeCommandIterator(ranges, command,
						concurrencyFactor, keyspace, consistencyLevel, queryStartNanoTime)), command.nowInSec(),
						command.selectsFullPartition(), command.metadata().enforceStrictLiveness());
	}
	...
	/**
	 * initiate a request/response session with each live node to check whether or
	 * not everybody is using the same migration id. This is useful for determining
	 * if a schema change has propagated through the cluster. Disagreement is
	 * assumed if any node fails to respond.
	 */
	public static Map<String, List<String>> describeSchemaVersions() {
		...
		// we're done: the results map is ready to return to the client. the rest is
		// just debug logging:
	 ...
	 * replica destinations for many ranges, so we need to restrict each scan to the
	 * specific range we want, or else we'd get duplicate results.
	 */
	static <T extends RingPosition<T>> List<AbstractBounds<T>> getRestrictedRanges(final AbstractBounds<T> queryRange) {
		...
		while (ringIter.hasNext()) {
			/*
			 * remainder can be a range/bounds of token _or_ keys and we want to split it
			 * with a token: - if remainder is tokens, then we'll just split using the
			 * provided token. - if remainder is keys, we want to split using
			 * token.upperBoundKey. For instance, if remainder is [DK(10, 'foo'), DK(20,
			 * 'bar')], and we have 3 nodes with tokens 0, 15, 30. We want to split
			 * remainder to A=[DK(10, 'foo'), 15] and B=(15, DK(20, 'bar')]. But since we
			 * can't mix tokens and keys at the same time in a range, we uses
			 * 15.upperBoundKey() to have A include all keys having 15 as token and B
			 * include none of those (since that is what our node owns). asSplitValue()
	...
	public static boolean shouldHint(InetAddress ep) {
		if (DatabaseDescriptor.hintedHandoffEnabled()) {
			...
			boolean hintWindowExpired = Gossiper.instance.getEndpointDowntime(ep) > DatabaseDescriptor
					.getMaxHintWindow();
	...
	/**
	 * Performs the truncate operatoin, which effectively deletes all data from the
	...
	public static void truncateBlocking(String keyspace, String cfname) throws UnavailableException, TimeoutException {
		...
		if (isAnyStorageHostDown()) {
			...
			throw new UnavailableException(ConsistencyLevel.ALL,
	...
	private static class ViewWriteMetricsWrapped extends BatchlogResponseHandler<IMutation> {
		public ViewWriteMetricsWrapped(AbstractWriteResponseHandler<IMutation> writeHandler, int i,
				BatchlogCleanup cleanup, long queryStartNanoTime) {
	...
	/**
	 * Like DroppableRunnable, but if it aborts, it will rerun (on the mutation
	 * stage) after marking itself as a hint in progress so that the hint
	 * backpressure mechanism can function.
	 */
	...
	/**
	 * HintRunnable will decrease totalHintsInProgress and targetHints when
	 * finished. It is the caller's responsibility to increment them initially.
	 */
	...
	public static Future<Void> submitHint(Mutation mutation, InetAddress target,
			AbstractWriteResponseHandler<IMutation> responseHandler) {
		return submitHint(mutation, Collections.singleton(target), responseHandler);
	}

	public static Future<Void> submitHint(Mutation mutation, Collection<InetAddress> targets,
			AbstractWriteResponseHandler<IMutation> responseHandler) {
